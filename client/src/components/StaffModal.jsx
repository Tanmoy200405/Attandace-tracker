...
const formatTimeForStorage = (value, fallback) => {
  const normalized = toTimeInputValue(value, fallback);
  const [rawHours, minutes] = normalized.split(":");
...
export const StaffModal = ({ staff, isOpen, onClose, onSuccess }) => {
  ...
  useEffect(() => {
    if (staff) {
      setFormData({
        ...
        expectedCheckIn: toTimeInputValue(staff.expectedCheckIn, "09:00"),
        expectedCheckOut: toTimeInputValue(staff.expectedCheckOut, "17:00"),
      });
    } else {
      setFormData({
        name: "",
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = {
      ...formData,
      expectedCheckIn: formatTimeForStorage(formData.expectedCheckIn, "09:00"),
      expectedCheckOut: formatTimeForStorage(
        formData.expectedCheckOut,
        "17:00",
      ),
    };
    try {
      if (staff?._id) {
        await api.staff.update(staff._id, payload);
      } else {
        ...
        <form onSubmit={handleSubmit} style={{ padding: "1.5rem" }}>
          ...
          >
            ...
            >
              ...
              >
                <div className="form-group">
                  <label className="form-label" htmlFor="entry-shift-time">
                    Entry Time
                  </label>
                  <input
                    id="entry-shift-time"
                ...
                <div className="form-group">
                  <label className="form-label" htmlFor="exit-shift-time">
                    Exit Time
                  </label>
                  <input
                    id="exit-shift-time"
